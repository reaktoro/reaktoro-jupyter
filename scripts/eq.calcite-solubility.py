...
# Function that defines the chemical problem corresponding to rainwater saturated with carbon-dioxide:
...
# Function that defines the chemical problem corresponding to open system (water saturated with carbon-dioxide
# partial pressure of the atmosphere pCO2 = 3.408. To convert the amount of CO2 gas from millimeters of mercury
# to parts per million we use the instruction on the
...
# where 44.01 g/mol is molar amount of CO2.
...
def water_co2_problem(system, T, P):
    ...
    problem.setPressure(P, "bar")
    problem.add("H2O", 1.0, "kg")
    problem.add("CO2", 0.010793, "mol")  # amount of carbon dioxide to saturate water
...
def solubility_of_calcite(problem, system):
    ...
    # Add 10mol of calcite
    state.setSpeciesAmount("Calcite", n0Calcite, "mol")

    # Equilibrate pure water with calcite
    solver.solve(state)
...
# Calculate solubilities of calcite in water and CO2-saturated rainwater for pressure P = 1 bar and save it in txt
# file:
...
# Check if obtained value correspond to the values of Wikipedia, where the solubility in water = 0.013 g/L (25 &deg;C):
# Below, we use 100.0869 g/mol as a molar mass of calcite:

print(f"Solubility of calcite in water (closed system) = {delta_calcite_water_P1[1]:.6f} mol/kgw = ... = "
...
# Calculate solubilities of calcite in water and CO2-saturated rainwater for pressure P = 100 bar and save it in txt
# file:
...
# The pressure corresponding to the partial pressure of the atmosphere pCO2 = 3.408 is equal to $39 = 10^{-3.408}$ Pa.
#
...
print(f"Solubility of calcite in open system = {delta_calcite_water_co2_P1[1]:.6f} mol/kgw = ... = "
   f"{delta_calcite_water_co2_P1[1] * 0.1000869 * 1e3:.6f} g/L")

# We see that the solubility of calcite in open system is about 4 times higher than in the closed system (0.000124
# mol/kgw or 0.012455 g/L).
